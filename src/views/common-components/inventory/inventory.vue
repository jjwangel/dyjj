...
<template>
  <div class="comm-contenter">
    <div class="contenter-struct">
      <Button type="primary" @click="refreshStructTree" long>刷新存货分类</Button>
    ...
    <div class="contenter-data">
      <Card shadow>
        <template slot="title">
          <Row>
            <Col span="6">
            <Input v-model="search_by_code" @on-change="handleSearchInTable" icon="ios-search" placeholder="存货档案编码..." style="width: 150px;"></Input>
            </Col>
            <Col span="6">
            <Input v-model="search_by_name" @on-change="handleSearchInTable" icon="ios-search" placeholder="存货档案名称..." style="width: 150px"></Input>
            </Col>
            <Col span="10">
            ...
            <Poptip placement="bottom" width="500">
              <Badge :count="getSelectedCount">
                <Button type="primary" shape="circle" icon="social-buffer-outline"></Button>
              </Badge>
              <div slot="content">
                <Table border size="small" height="300" :columns="selected_column" :data="selected_data"></Table>
...
<script>
  ...
  export default {
    ...
    methods: {
      init() {
        getInventoryClassData().then((response) => {
      ...
      getDetailInfoByCode(class_code) {
        this.detail_loading = true;
        getInventoryDetailInfo(class_code).then((response) => {
          if (response.data.retrun_code === "000000") {
            this.detail_data = response.data.inv_data;
      ...
      handleSearchInTable() {
        this.detail_data = util.searchByKey(this.detail_init_data, {
          inv_code: this.search_by_code,
          inv_name: this.search_by_name
        });
      },
      handleSearchAllInfo() {
        if (this.search_all_info.trim() != "") {
          ...
          getInventoryDetailInfo(this.search_all_info.trim()).then((response) => {
            if (response.data.retrun_code === "000000") {
              this.detail_data = response.data.inv_data;
              this.detail_init_data = this.detail_data;
            } else {
      ...
      handleSelectOne(selection, row) {
        this.selected_data = util.addObjToArray(this.selected_data, Array.of(row), "inv_code");
      },
      handleSelectChange(selection) {
        if (selection.length == this.detail_data.length) {
          this.selected_data = util.addObjToArray(this.selected_data, this.detail_data,
            "inv_code");
        } else if (selection.length == 0) {
          this.selected_data = util.removeObjFromArray(this.selected_data, this.detail_data,
            "inv_code");
        }
      },
      handleCancelSelectOne(selection, row) {
        this.selected_data = util.removeObjFromArray(this.selected_data, Array.of(row), "inv_code");
      },
      handleDelOneSelected(row) {
        this.selected_data = util.removeObjFromArray(this.selected_data, Array.of(row), "inv_code");
        let i = this.detail_data.findIndex(item => {
          return item.inv_code == row.inv_code;
        });
        if (i >= 0) {
