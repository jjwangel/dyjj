    ...
    <div class="contenter-data">
      <Card shadow>
        <template slot="title">
          <Row>
            <Col span="6">
            <Input v-model="search_by_code" @on-change="handleSearchInTable" icon="ios-search" placeholder="业务员编码..." style="width: 150px;"></Input>
            </Col>
            <Col span="6">
            <Input v-model="search_by_name" @on-change="handleSearchInTable" icon="ios-search" placeholder="业务员名称..." style="width: 150px"></Input>
            </Col>
            <Col span="10">
            ...
            <Poptip placement="bottom" width="500">
              <Badge :count="getSelectedCount">
                <Button type="primary" shape="circle" icon="person"></Button>
              </Badge>
              <div slot="content">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getDetailInfoByCode(dep_code) {
        this.detail_loading = true;
        getPersonInfo(dep_code).then((response) => {
          if (response.data.retrun_code === "000000") {
            this.detail_data = response.data.psn_data;
      ...
      handleSearchInTable() {
        this.detail_data = util.searchByKey(this.detail_init_data, {
          psn_code: this.search_by_code,
          psn_name: this.search_by_name
        });
      },
      handleSearchAllInfo() {
        if (this.search_all_info.trim() != "") {
          ...
          getPersonInfo(this.search_all_info.trim()).then((response) => {
            if (response.data.retrun_code === "000000") {
              this.detail_data = response.data.psn_data;
              this.detail_init_data = this.detail_data;
            } else {
      ...
      handleSelectOne(selection, row) {
        this.selected_data = util.addObjToArray(this.selected_data, Array.of(row), "psn_code");
      },
      handleSelectChange(selection) {
        if (selection.length == this.detail_data.length) {
          this.selected_data = util.addObjToArray(this.selected_data, this.detail_data, "psn_code");
        } else if (selection.length == 0) {
          this.selected_data = util.removeObjFromArray(this.selected_data, this.detail_data, "psn_code");
        }
      },
      handleCancelSelectOne(selection, row) {
        this.selected_data = util.removeObjFromArray(this.selected_data, Array.of(row), "psn_code");
      },
      handleDelOneSelected(row) {
        this.selected_data = util.removeObjFromArray(this.selected_data, Array.of(row), "psn_code");
        let i = this.detail_data.findIndex(item => {
          return item.psn_code == row.psn_code;
        });
        if (i >= 0) {
