    ...
    <div class="contenter-data">
      <Card shadow>
        <template slot="title">
          <Row>
            <Col span="6">
            <Input v-model="search_by_code" @on-change="handleSearchInTable" icon="ios-search" placeholder="存货分类编码..." style="width: 150px;"></Input>
            </Col>
            <Col span="6">
            <Input v-model="search_by_name" @on-change="handleSearchInTable" icon="ios-search" placeholder="存货分类名称..." style="width: 150px"></Input>
            </Col>
            <Col span="10">
            ...
            <Poptip placement="bottom" width="500">
              <Badge :count="getSelectedCount">
                <Button type="primary" shape="circle" icon="ios-keypad-outline"></Button>
              </Badge>
              <div slot="content">
  ...
  export default {
    ...
    methods: {
      ...
      getDetailInfoByCode(class_code) {
        ...
        getInventoryClassDetailInfo(class_code).then((response) => {
          if (response.data.retrun_code === "000000") {
            this.detail_data = response.data.class_data;
      ...
      handleSearchInTable() {
        this.detail_data = util.searchByKey(this.detail_init_data, {
          class_code: this.search_by_code,
          class_name: this.search_by_name
        });
      },
      handleSearchAllInfo() {
        if (this.search_all_info.trim() != "") {
          ...
          getInventoryClassDetailInfo(this.search_all_info.trim()).then((response) => {
            if (response.data.retrun_code === "000000") {
              this.detail_data = response.data.class_data;
              this.detail_init_data = this.detail_data;
            } else {
      ...
      handleSelectOne(selection, row) {
        this.selected_data = util.addObjToArray(this.selected_data, Array.of(row), "class_code");
      },
      ...
      handleCancelSelectOne(selection, row) {
        this.selected_data = util.removeObjFromArray(this.selected_data, Array.of(row), "class_code");
      },
      handleDelOneSelected(row) {
        this.selected_data = util.removeObjFromArray(this.selected_data, Array.of(row), "class_code");
        let i = this.detail_data.findIndex(item => {
          return item.class_code == row.class_code;
        });
        if (i >= 0) {
